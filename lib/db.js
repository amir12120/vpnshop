...
CREATE TABLE IF NOT EXISTS orders (
  ...
  -- CUSTOM orders (خرید به مقدار دلخواه): plan_id references a hidden auto
  -- plan; volume/duration/price live on the order itself. NULL for
  -- plan-based orders (values then come from the plan).
  volume_gb REAL,
...
(() => {
  ...
  // custom-quantity orders (فروش به مقدار دلخواه): per-order snapshot
  for (const c of ['volume_gb REAL', 'duration_days INTEGER', 'price_toman INTEGER']) {
    const name = c.split(' ')[0];
    if (!ocols.includes(name)) db.exec(`ALTER TABLE orders ADD COLUMN ${c}`);
  }
