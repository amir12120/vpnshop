...
// Random hex token used as the subscription path (subId) — never the email.
...
class SanayiClient {
  ...
  // Current cumulative traffic counters (bytes) for one client email.
  // 3x-ui v3: GET /panel/api/clients/traffic/:email — obj is the client's
  // traffic record { email, up, down, total, expiryTime, enable, ... }.
  // total = 0 means unlimited volume; up/down are cumulative since creation.
  async getClientTraffic(email) {
    const res = await this._authed(`${PANEL_PATH}/clients/traffic/${encodeURIComponent(email)}`);
    if (!res.json || !res.json.success) {
      throw new Error(`traffic lookup failed for "${email}" (HTTP ${res.status})`);
    }
    const t = res.json.obj || {};
    return {
      email: t.email || email,
      up: Number(t.up) || 0,
      down: Number(t.down) || 0,
      total: Number(t.total) || 0,           // 0 = unlimited
      expiryTime: Number(t.expiryTime) || 0, // epoch ms; 0 = never expires
      enable: t.enable !== false,
    };
  }

  // True if `email` exists in the clients list of ANY of the given inbounds.
  // 3x-ui returns inbound.settings as a JSON string (v2 era) or as a plain
  // object (current v3) — tolerate both.
  _clientInInbounds(inbounds, ids, email) {
    return inbounds.some((ib) => {
      if (!ids.includes(ib.id)) return false;
      let settings = ib.settings;
      try { if (typeof settings === 'string') settings = JSON.parse(settings); } catch { return false; }
      const clients = (settings && settings.clients) || [];
      return clients.some((c) => c.email === email);
    });
  }
  ...
  // (one call per inbound). After a reported success we RE-READ the panel and
  // verify the client actually landed in at least one inbound — some panels
  // answer success while silently skipping incompatible inbounds, which would
  // otherwise deliver configs for a client that does not exist.
  async addClient({ inboundIds, email, uuid, totalGB, expiryTime, limitIp = 2, subId }) {
    const ids = [].concat(inboundIds).map(Number).filter(Boolean);
    if (!ids.length) throw new Error('at least one inbound is required');
    // The subscription id is a separate RANDOM path (never the customer's
    // email/account name) so the sub link does not leak the account name.
    // Sanayi's own panel settings also auto-generate subIds when empty.
    const sid = subId || randomToken(10);
    const client = {
      ...
      subId: sid,                     // subscription id — panel uses this for /sub/<subId>
      flow: '',
    };
    ...
    if (v3.status !== 404 && v3.status !== 405 && v3.status !== 501) {
      if (v3.json && v3.json.success) {
        return this._assertClientCreated(email, ids);
      }
  ...
  // Re-read the panel and confirm the client landed in at least one inbound.
  async _assertClientCreated(email, ids) {
    const inbounds = await this.listInbounds();
    if (!this._clientInInbounds(inbounds, ids, email)) {
      throw new Error(`panel confirmed the add but client "${email}" was NOT found on any inbound — check the API token has write access and the inbound ids are correct`);
    }
    return true;
  }
  ...
  async getClientLinks({ inboundIds, email, subId }) {
    ...
    const inbounds = await this.listInbounds();
    // NEVER fabricate links for a client that is not really on the panel —
    // that is exactly the "delivered config does not work" failure mode.
    if (!this._clientInInbounds(inbounds, ids, email)) {
      throw new Error(`client "${email}" was not found on the panel inbounds ${ids.join(', ')} — it was not created, so nothing was delivered`);
    }
    const publicHost = hostnameOf(this.subUrl) || hostnameOf(this.baseUrl) || '';

    // v3: ask the panel for official subscription + client links; links/:email
    // already spans every attached inbound.
    ...
    if (subRes.status !== 404 && subRes.json && subRes.json.success) {
      ...
      if (!links.length) {
        for (const id of ids) {
          ...
          if (ib) links = links.concat(buildShareLinks(ib, { id: '', email, subId: sid }, publicHost));
        }
      }
    ...
    for (const id of ids) {
      ...
      try {
        ...
        client = (settings && settings.clients || []).find((c) => c.email === email);
      } catch { /* ignore */ }
      links = links.concat(buildShareLinks(inbound, client || { id: '', email, subId: sid }, publicHost));
    }
    return { subUrl: publicSub, links, client: null };
  }
}

// Best-effort hostname of a base/sub URL ('' when missing/invalid).
function hostnameOf(url) {
  try { return new URL(url).hostname || ''; } catch { return ''; }
}

// Build vless/vmess/trojan share links from an inbound definition.
function buildShareLinks(inbound, client, publicHost = '') {
  const links = [];
  let stream = {};
  ...
  // The address a client should connect to. Priority: the shop panel's public
  // sub address (operator-set, correct even behind a tunnel), then the panel
  // base URL host, then the inbound's public listen IP. If nothing resolvable
  // we emit no link rather than a broken "@SERVER" placeholder.
  const candidates = [publicHost, hostnameOf(inbound._baseUrl)];
  let address = candidates.find((h) => h && !isPrivateIp(h)) || '';
  if (!address && inbound.listen && inbound.listen !== '0.0.0.0' && !isPrivateIp(inbound.listen)) {
    address = inbound.listen;
  }
  if (!address) address = candidates.find(Boolean) || '';
  if (!address) return [];

  const tag = inbound.remark || inbound.tag || `in-${inbound.id}`;

  if (inbound.protocol === 'vless') {
    links.push(`vless://${client.id}@${address}:${port}?${query}#${encodeURIComponent(tag)}`);
  } else if (inbound.protocol === 'vmess') {
    const vm = { v: '2', ps: tag, add: address, port, id: client.id,
    ...
    links.push(`trojan://${client.id}@${address}:${port}?${query}#${encodeURIComponent(tag)}`);
  }
  return links;
}
