...
const BUCKET_SECS = { hour: 3600, day: 86400, month: 86400 * 30 };
    ...
    for (const s of rows) {
      if (pu !== null && pd !== null) {
        ...
        if (du > 0 || dd > 0) {
          ...
          if (idx >= 0 && idx < points) out[idx] += du + dd;
        }
      }
      pu = s.up; pd = s.down;
    }
  }
  return { size, points, start, values: out, keys: keys.length };
}
